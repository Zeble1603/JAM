...
router.post('/signup', (req, res, next) => {
	...
	User.findOne({ email })
		.then((foundUser) => {
			...
			return User.create({ email, password: hashedPassword, username, type });
		})
		.then((createdUser) => {
			// Deconstruct the newly created user object to omit the password
			// We should never expose passwords publicly
			const { email, username, _id, type } = createdUser;

			// Create a new object that doesn't expose the password
			const user = { email, username, _id, type };
	...
	User.findOne({ email })
		.then((foundUser) => {
			...
			if (passwordCorrect) {
				...
				const { _id, email, username, type } = foundUser;
